class Order {
    ...
    sortItemsByPrice(sortingOrder) {
        ...
        const index = check.findIndex((index) => index === sortingOrder)
        if (index !== -1) {
            if (check[index] === 'descending') {
                const sorted = this.items.sort((a, b) => a.price - b.price);
                return sorted;
            }
            if (check[index] === 'ascending') {
                const sorted = this.items.sort((a, b) => a.price + b.price);
                return sorted;
            }
        } return []
...
console.log(myOrder.addItem({ name: "Headphones", price: -50, quantity: 1 }));

console.log(
    "remove item----------------------------------------------------------------"
);
//remove item
// console.log(myOrder.removeItem("Laptop")); // Removes Laptop if it exists 
// console.log(myOrder.removeItem("keyboard")); // Removes Keyboard if it exists 
// console.log(myOrder.removeItem("NonExistentItem"));  // Tries to remove a non-existent item 
// console.log(myOrder.removeItem(""));  // Tries to remove an item with an empty name 
// console.log(myOrder.removeItem("Mouse")); // Removes Mouse if it exists
